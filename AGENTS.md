...
| `start.zsh`, scripts racine | `README.md`, `AGENTS.md` (commandes) |
...
│   │   ├── effects.js          ← Particules, confettis, sons (servi sous /shared)
...
`app.js` orchestre les autres et expose `window.App`. `effects.js` (partagé) expose `window.Confetti` et `window.Sounds`. Ne pas modifier cet ordre.
...
Le script `start.zsh` à la racine :
- vérifie **Node.js >= 18**
...
| `shared/modules/plinko` | `generateBoard`, `simulateDropSeeded`, `generateMinigameLayout`, `rollMinigameAmount` | Plateau Plinko + chute discrète + mini-jeux couteau/voleur |
...
| `shared/client/effects.js` | `window.Confetti`, `window.Sounds` | Effets visuels + sons (client) |
...
| `shared/client/player-faces.js` | `window.PlayerFaces` | Visages joueurs (variante idle/win/lose, tailles sm→xxl) + repli initiales + `getCutoutUrl` + têtes qui tombent (`dropHead`/`rainHeads`) |
...
Le hub (`esp32SensorService.js`) calibre (5 s), calcule un seuil par capteur (`baseline × ratio`, défaut 55 %, réglable et persisté), détecte les balles et appelle le même flux que `POST /api/trigger`. Dashboard : `/sensors`.
